...
public class KnightsTour {
    ...
    public boolean moveWithAnimation(int x, int y, int m, GUI gui) {
        ...
        // moves that can be made from this location recursively.
            ...
            neighbors.sort((p1, p2) -> Integer.compare(getNumNeighbors(p1), getNumNeighbors(p2)));
    ...
    private ArrayList<Point> getNeighbors(Point p) {
        ...
        for (Point move : moves) {
            Point next = new Point((int) p.getX() + (int) move.getX(), (int) p.getY() + (int) move.getY());
    ...
    public void solveWithAnimation(final int x, final int y, final GUI gui, final JTextArea t) {
        // setup fields
        stack = new Stack<>();
